...
`);
// 2026-08-22, next round — two roadmap-item-2/4 gaps closed together.
//
// Gap A (item 2): scoreDraftCopy() has only ever been reachable from
// creative_jobs, via POST /api/creative-jobs/:id/score. The Marketing
// Function Project generators (Social Batch, Email Sequence, Landing Page,
// Press Release, Internal Comms, One-Pager, Analytics Brief) have their own
// output — project.masterContent and its derivatives, held client-side —
// but that output only ever gets the deterministic buildContentChecks()
// heuristic (see portal.html), never the real AI-judged relevance/
// compliance score every creative_jobs draft gets. content_score_history
// is the log both surfaces now write to: sourceType distinguishes
// 'creative_job' rows (unchanged behavior, now ALSO logged here alongside
// the existing single-latest-score columns on creative_jobs itself) from
// the new 'campaign_copy' rows the MFP workspace produces. sourceKey is a
// free-form string identifying WHAT was scored within its source (a jobId
// for creative_job rows; a string like 'social-batch:master' or
// 'landing-page:derivative:adClickThrough' for campaign_copy rows) — this
// is also what makes item 3 (score history) real: querying by sourceKey
...
`);
// Gap B (item 4): runCandidateInterview() was already built generically
// (any campaign+account, nothing copy-specific except the angle briefs —
// see its own comment), but the only endpoint that calls it is job-scoped
// (POST /api/creative-jobs/:id/interview), and creative_job_interviews.jobId
// is NOT NULL with a real FK to creative_jobs — a Marketing Function
// Project's copy has no creative_jobs row to attach to (same reasoning
// that already justified account_voice_interviews as its own table rather
// than forcing a null jobId onto that one). campaign_copy_interviews is the
// campaign-scoped sibling: same append-only/select-once shape, keyed by
// campaignId + sourceKey (same sourceKey convention as content_score_history
// above, so a given piece of MFP copy's scores and its interview history
// line up under the same key) instead of jobId. No workingCopy-equivalent
// exists to auto-apply a selection into server-side — MFP master/derivative
...
function logScoreHistory(accountId, campaignId, sourceType, sourceKey, result){
  const now = new Date().toISOString();
  db.prepare(`INSERT INTO content_score_history (id, accountId, campaignId, sourceType, sourceKey, relevanceScore, complianceScore, note, complianceFlagsJson, mode, scoredAt, createdAt)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`)
...
function scoreComplianceHeuristically(copyText, account, campaign){
  const flags = [];
  ...
  // 2026-08-22, next round (item 10) — the same "Mandatory Phrase present"
  // check the frontend's buildContentChecks() validator already runs,
  // folded into the real scorer's compliance pass too, so a copy piece
  // that fails this doesn't just get flagged by one validator and not the
  // other. A missing mandatory phrase IS a compliance issue by this
  // account's own definition of the word (they set it, it's required),
  // not a relevance one — it belongs in this function, not the heuristic
  // relevance scorer above.
...
${copyText.slice(0, 2000)}`;
  ...
  const comp = scoreComplianceHeuristically(copyText, account, campaign);
...
// pasted into this codebase, any doc, or any delivered file — see the
// ".env, not code" note further down) and asked to wire them in. This
// replaces the permanent "architected for, not yet credentialed" placeholder
// list with a real registry: each vendor becomes a live candidate the
// MOMENT its own env var is actually set on the deployment (checked at
// request time via `process.env[v.envVar]`, same pattern this file already
// uses for ANTHROPIC_API_KEY everywhere else), and silently stays an honest
// "not configured" placeholder otherwise — no code change needed the day a
// key is added or removed. Shared by BOTH panels that reuse this exact
// vendor pool: the copywriting interview (runCandidateInterview below) and
// the Brand Voice contest (runBrandVoiceContest above) — a vendor
// configured for one is configured for both, since it's the same
// underlying credential either way.
//
// IMPORTANT — this integration is UNTESTED against live vendor traffic.
// Every other AI-backed feature in this codebase was verified end-to-end
// against a real API call (including, this same round, deliberately
// exercising Anthropic's real failure path with an invalid key). That
// discipline isn't possible here without handling the actual secret values,
// which this file is deliberately written to never do. What IS verified:
// `node --check` syntax, and that each request/response shape below matches
// each vendor's own published API documentation as of this write. The
// first real call from a deployment with real keys set is the true test —
// if a response shape below is stale by the time that happens (fast-moving
// vendor APIs), the honest failure path (HTTP-status/parse-error message
// per candidate, same as every other candidate failure) is what surfaces,
// not a crash.
const INTERVIEW_VENDOR_REGISTRY = [
  ...
  { key: 'xai-grok', label: 'Grok (xAI)', vendor: 'xAI', envVar: 'XAI_API_KEY', model: process.env.XAI_MODEL || 'grok-4.6' },
  { key: 'perplexity', label: 'Perplexity', vendor: 'Perplexity', envVar: 'PERPLEXITY_API_KEY', model: process.env.PERPLEXITY_MODEL || 'sonar-pro' }
];
// Every vendor's default MODEL name is env-overridable (OPENAI_MODEL /
// GEMINI_MODEL / XAI_MODEL / PERPLEXITY_MODEL) precisely because these are
// the fastest-moving part of this integration — a vendor deprecating a
// model doesn't require a code change, just an env var update.

// One vendor's raw completion call, prompt in / text out — deliberately
// separate from JSON-shape parsing (see generateVendorInterviewCopy /
// generateVendorBrandVoiceCopy below) since the two panels that share this
// vendor pool ask each vendor for two different JSON shapes from the same
...
async function callVendorForText(vendorKey, prompt){
  ...
  if (vendorKey === 'google-gemini'){
    // Google's own documented auth pattern for this API is the key as a URL
  ...
  if (vendorKey === 'xai-grok'){
    // x.ai's Responses API — per Todd's own supplied curl example:
...
// campaign_copy_interviews, account_voice_interviews all count toward the
// same cap — same underlying cost regardless of which panel triggered it).
// Deliberately NOT a dollar-cost or credit-balance system — that's blocked
// on real paid-tier pricing still being TBD (see PAID_TIERS placeholder
// entries) and is flagged in the runbook as the natural next step once
// pricing exists, folding into the existing PREMIUM_CAP_TIERS "asks"
// allowance rather than a third, parallel metering mechanism. This is
// purely a safety ceiling: cheap to build, no pricing dependency.
// Configurable via INTERVIEW_WEEKLY_CAP env var; defaults to 20/week/account.
...
// Returns null if under the cap, or a ready-to-send 429 response body if
...
// The panel itself — POST /api/creative-jobs/:id/interview and
// POST /api/campaigns/:id/copy-interview both call this. Requires
// ANTHROPIC_API_KEY (same gate as generateMessagingCopyViaAI) — without it
// there is no real brief to run multiple ways at all, so this returns
// available:false with an honest note rather than a comparison with nothing
// real in it. Runs the 3 Anthropic angles AND every vendor from
// INTERVIEW_VENDOR_REGISTRY whose env var is actually set, in parallel,
// scores every real candidate with the SAME scoreDraftCopy() used by the
// item-2 pre-decision score — this panel is that one judgment mechanism
// applied to every candidate, not a second, different one — then ranks by
// the average of relevance and compliance across ALL live candidates
// (Anthropic angles and vendor candidates compete on equal footing). A
// vendor whose env var isn't set still appears as an honest "not
...
async function runCandidateInterview(campaign, account){
  ...
  const buildCandidate = (key, label, vendor, model, gen, score) => {
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && parts.length === 4 && parts[0] === 'api' && parts[1] === 'creative-jobs' && parts[3] === 'score'){
      ...
      // 2026-08-22, next round (item 3) — also append to the shared history
      // log, keyed by this job so past scores for this exact job are
      // queryable via GET .../score-history below, not just the latest.
      if (result.mode !== 'none') logScoreHistory(job.accountId, job.campaignId, 'creative_job', `job:${jobId}`, result);
      return sendJson(res, 200, {
    ...
    // POST /api/campaigns/:id/score-copy — 2026-08-22, next round (item 2
    // continued). The Marketing Function Project generators' equivalent of
    // POST /api/creative-jobs/:id/score above — same scoreDraftCopy() call,
    // same real-AI-with-heuristic-fallback behavior, just for copy that
    // lives client-side on a campaign's project state rather than on a
    // creative_jobs row. Body: { copy: string (required), sourceKey: string
    // (required — identifies which MFP master/derivative this is, e.g.
    // 'social-batch:master' or 'landing-page:derivative:adClickThrough';
    // this is what makes the score-history and copy-interview endpoints
    // below able to group scores/interviews by the exact piece of copy they
    // belong to). Does NOT persist onto the campaign row itself — unlike a
    // creative_jobs draft, there's no one "current" MFP copy for a given
    // sourceKey (a project can be regenerated repeatedly), so every call
    // both returns its result AND logs it to content_score_history; the
    // frontend is the source of truth for "what's the latest," same as it
    // already is for project.masterContent itself.
    if (req.method === 'POST' && parts.length === 4 && parts[0] === 'api' && parts[1] === 'campaigns' && parts[3] === 'score-copy'){
      const campaignId = decodeURIComponent(parts[2]);
      const campaign = db.prepare('SELECT * FROM campaigns WHERE id = ?').get(campaignId);
      if (!campaign) return sendJson(res, 404, { error: 'campaign not found' });
      if (!requireAccount(req, res, campaign.accountId)) return;
      ...
      const copyText = typeof body.copy === 'string' ? body.copy : '';
      const result = await scoreDraftCopy(copyText, campaign, account);
      const scoredAt = result.mode !== 'none'
        ? logScoreHistory(campaign.accountId, campaignId, 'campaign_copy', body.sourceKey, result)
        : new Date().toISOString();
      return sendJson(res, 200, {
        relevanceScore: result.relevanceScore, complianceScore: result.complianceScore,
        note: result.note || null, flags: result.flags || [], mode: result.mode, scoredAt
      });
    }

    // GET /api/campaigns/:id/score-history?sourceKey=... — 2026-08-22, next
    // round (item 3). Returns every content_score_history row for this
    // campaign, most recent first; sourceKey narrows to one piece of copy
    // (a job or an MFP master/derivative). Read-only, works for both
    // 'creative_job' and 'campaign_copy' rows — the same history log serves
    // both surfaces (see content_score_history's own comment for why one
    // shared table).
    if (req.method === 'GET' && parts.length === 4 && parts[0] === 'api' && parts[1] === 'campaigns' && parts[3] === 'score-history'){
      const campaignId = decodeURIComponent(parts[2]);
      const campaign = db.prepare('SELECT * FROM campaigns WHERE id = ?').get(campaignId);
      if (!campaign) return sendJson(res, 404, { error: 'campaign not found' });
      if (!requireAccount(req, res, campaign.accountId)) return;
      ...
      const rows = sourceKey
        ? db.prepare('SELECT * FROM content_score_history WHERE campaignId = ? AND sourceKey = ? ORDER BY scoredAt DESC').all(campaignId, sourceKey)
        : db.prepare('SELECT * FROM content_score_history WHERE campaignId = ? ORDER BY scoredAt DESC').all(campaignId);
      return sendJson(res, 200, { history: rows.map(r => ({
        id: r.id, sourceType: r.sourceType, sourceKey: r.sourceKey, relevanceScore: r.relevanceScore,
        complianceScore: r.complianceScore, note: r.note, flags: (() => { try { return JSON.parse(r.complianceFlagsJson) || []; } catch (e){ return []; } })(),
        mode: r.mode, scoredAt: r.scoredAt
      })) });
    }
    ...
    // convention as score-copy above).
    if (req.method === 'POST' && parts.length === 4 && parts[0] === 'api' && parts[1] === 'campaigns' && parts[3] === 'copy-interview'){
      const campaignId = decodeURIComponent(parts[2]);
      const campaign = db.prepare('SELECT * FROM campaigns WHERE id = ?').get(campaignId);
      if (!campaign) return sendJson(res, 404, { error: 'campaign not found' });
      if (!requireAccount(req, res, campaign.accountId)) return;
    ...
    // MFP copy lives client-side — so this only records the selection and
    // hands back the winning candidate's text for the frontend to apply.
    if (req.method === 'POST' && parts.length === 6 && parts[0] === 'api' && parts[1] === 'campaigns' && parts[3] === 'copy-interview' && parts[5] === 'select'){
      const campaignId = decodeURIComponent(parts[2]);
      const interviewId = decodeURIComponent(parts[4]);
      const campaign = db.prepare('SELECT * FROM campaigns WHERE id = ?').get(campaignId);
      if (!campaign) return sendJson(res, 404, { error: 'campaign not found' });
    ...
    // workspace, so past interview runs for a piece of copy are visible
    // there directly rather than only in the Ops Console (which only ever
    // showed creative_job_interviews, not this campaign-scoped table).
    // Candidates are redacted the same way the live-run response is —
    // history is still client-facing data, same redaction rules apply.
    if (req.method === 'GET' && parts.length === 4 && parts[0] === 'api' && parts[1] === 'campaigns' && parts[3] === 'copy-interviews'){
      const campaignId = decodeURIComponent(parts[2]);
      const campaign = db.prepare('SELECT * FROM campaigns WHERE id = ?').get(campaignId);
      if (!campaign) return sendJson(res, 404, { error: 'campaign not found' });
      if (!requireAccount(req, res, campaign.accountId)) return;
      const sourceKey = url.searchParams.get('sourceKey');
      const rows = sourceKey
        ? db.prepare('SELECT * FROM campaign_copy_interviews WHERE campaignId = ? AND sourceKey = ? ORDER BY createdAt DESC').all(campaignId, sourceKey)
        : db.prepare('SELECT * FROM campaign_copy_interviews WHERE campaignId = ? ORDER BY createdAt DESC').all(campaignId);
      return sendJson(res, 200, { interviews: rows.map(r => {
        let candidates = [];
        try { candidates = JSON.parse(r.candidatesJson) || []; } catch (e){ candidates = []; }
        return {
    ...
    if (req.method === 'POST' && parts.length === 4 && parts[0] === 'api' && parts[1] === 'creative-jobs' && parts[3] === 'interview'){
      ...
      const account = db.prepare('SELECT * FROM accounts WHERE accountId = ?').get(job.accountId);
      if (!campaign || !account) return sendJson(res, 404, { error: 'campaign or account not found for this job' });
      const capCheck1 = checkInterviewWeeklyCap(job.accountId);
      if (capCheck1) return sendJson(res, 429, capCheck1);
      const result = await runCandidateInterview(campaign, account);
      if (!result.available){
        return sendJson(res, 200, { available: false, note: result.note, interviewId: null, recommendedKey: null, candidates: [] });
      }
